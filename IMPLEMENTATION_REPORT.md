...
## 2026-01-21 プロジェクト保留判断

### 経緯

オフセットリセット後の検証で、BME680の氷点下環境での測定精度に根本的な問題が発覚。

### 検証結果

**Ambientグラフ分析（2026-01-21 18:00〜翌11:00）**:
- 室内（18:00頃）: 温度約21°C（妥当）
- 屋外移動後（20:00頃）: 温度が一瞬-4°Cに下がるが、すぐに約21°Cに戻る
- その後（21:00以降）: 外気温は氷点下なのに、温度は約21°Cで固定

**矛盾点**:
オフセットを0にリセットしたにも関わらず、室内では妥当な値（21°C）、屋外では異常な値（21°C固定）を示した。
もしオフセットだけの問題であれば、室内でも高すぎる値を示すはずである。

### 原因調査（Web検索）

BME680/BME280の寒冷環境での問題が広く報告されている：

1. **自己発熱問題**
   - センサーICの動作による発熱（MCU、WiFi通信）
   - 基板温度が周囲温度より高くなる
   - 通常3-4°C高く測定されるが、氷点下では差が極端に大きくなる

2. **氷点下での特有の問題**
   - 0°C付近で不安定になる
   - 氷点下に移行する際、温度値が約2度ジャンプする現象
   - -3°Cの環境で-0.5°Cと表示されるケース（2-4°Cの誤差）

3. **参考情報**
   - [Adafruit BME680 Issue #34](https://github.com/adafruit/Adafruit_BME680/issues/34)
   - [pimoroni bme680-python Issue #11](https://github.com/pimoroni/bme680-python/issues/11)
   - [Arduino Forum: BME280 inaccurate temperature](https://forum.arduino.cc/index.php?topic=558266.0)
   - [Let's Control It: BMP280 wrong temp at below 0](https://www.letscontrolit.com/forum/viewtopic.php?t=2690)

### M5Stack Core2 + ENV2との比較

同時刻のM5Stack ENV2プローブデータ（data94635.csv）:
- 8:22頃（屋外移動直後）: 7.95°C → 1.05°C → -0.89°C（順調に低下）
- 9:00-11:42: -0.5°C ～ -2.1°C（安定して氷点下を測定）

**結論**: M5Stack Core2 + ENV2プローブはセンサーが本体から物理的に離れているため、MCUやバッテリーの発熱の影響を受けにくく、氷点下環境でも安定した測定が可能。

### 保留判断の理由

**BME680は氷点下の屋外気温測定には不向き**と判断：

1. センサー自己発熱 + Pico Wの発熱 + 氷点下での非線形応答により、氷点下環境で約20°C以上の誤差が発生
2. これはオフセット補正で対応できる範囲を超えている
3. 屋外・氷点下での測定にはDS18B20などの専用センサーが推奨される

### 今後の方針

- **本プロジェクト（RPpicow_BME680）**: 保留
- **屋外気象観測**: M5Stack Core2 + ENV2プローブを使用
- **BME680の活用**: 室内での空気質測定（ガスセンサー活用）、温暖な環境での使用に限定

### 学んだこと

1. BME680は高機能だが、寒冷地屋外での温度測定には向かない
2. プローブ型センサー（本体から物理的に離れている）の方が外気温測定に適している
3. センサー選定は使用環境を十分考慮する必要がある

---

**プロジェクトステータス**: 保留
**保留日**: 2026-01-21
**再開条件**: 室内空気質測定など、BME680の特性を活かせる用途が見つかった場合

---

## 2026-01-21 ラズパイ4 BME680（Room426）IAQ機能追加

### 概要

学内LAN接続のラズパイ4（10.18.52.46）で動作するBME680センサーに、簡易IAQ（室内空気質）計算機能を追加。

### 実施内容

1. **簡易IAQ計算ロジックの実装**
   - ガス抵抗値と湿度からIAQスコア（0-500）を計算
   - ガス抵抗の寄与: 75%、湿度の寄与: 25%
   - 起動後3分間でガスベースラインを自動計算
...
3. **Ambientボード作成**
   - チャンネル: Room428(BME680)（ID: 82280）
   - ボード「room426(BME680)」を作成
   - 気圧、温度・湿度、IAQの3チャートを配置
...
| 0-50 | 優良 (Excellent) |
| 51-100 | 良好 (Good) |
| 101-150 | 軽度汚染 (Lightly Polluted) |
| 151-200 | 中度汚染 (Moderately Polluted) |
| 201-300 | 重度汚染 (Heavily Polluted) |
| 301-500 | 危険 (Severely Polluted) |
...
- `/home/pi4/Desktop/bme680_test/bme680/bme680_read_01.py`
- バックアップ: `bme680_read_01.py.backup_20260121`

### 備考

- プログラムは`/etc/rc.local`で自動起動設定済み
- ネットワーク不安定時は再起動で復旧
